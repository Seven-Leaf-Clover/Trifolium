					...
					G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.3, blockable = false, func = function() --destroys itcard
						...
						return true end })) 
							G.E_MANAGER:add_event(Event({trigger = 'before', delay = 0.0, func = function()
								...
								colour = G.C.RED,
								card = card,
								message = localize("k_frame7") --VAL = 16 only when the odds hit and creates the Blueprint
									}
								else
								local roll_int = math.floor(roll * 6) + 1 --convert to a number between 1-16 and shows the rolled value below Joker
